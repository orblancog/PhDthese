\chapter{Chromaticity in the FFS}
...
\end{table}
It is shown in \cite{Brown:1987} that first order dependence with the energy spread, $\delta$, is cancelled if the lattice is conceived as a telescope where the matrix elements are as in Eq. (\ref{eq:telescope}), $M_x, M_y$ are the magnifications in horizontal and vertical planes. Also, if second and higher order components are considered in the transfer map, as in Eq. (\ref{eq:tmap}), then, the telescope properties show that the main contribution to beam size is chromaticity given by the elements $T_{126},\,T_{346}$.\par
Therefore, the Final Focus Section (FFS) has been conceived as a telescope to demagnify the beam size with minimum effect of energy spread, where the main issue is the large chromaticity generated at the Final Doublet (FD), the last pair of quadrupoles which focus the beam to the Interaction Point (IP).
\begin{equation}
R=
 \begin{pmatrix}
   M_x & 0 & 0 & 0 \\
...
\end{equation}
\begin{equation}
x_i=\sum_{j=1}^6R_{ij}x_j+\sum_{j,k=1}^6T_{ijk}x_jx_k+\cdots\qquad x_i\in x,x',y,y',\tau,\delta\label{eq:tmap}
\end{equation}

\section{Chromaticity minimization in the Final Doublet (FD)}
% \section{Thin lens approximation}
Being the chromaticity as in Eq. (\ref{eq:chrom}), where the components $T_{116},\,T_{336}$ have been cancelled by the telescope properties, here I find a configuration of distances and strengths that minimize the effect.
\begin{equation}
...
\begin{figure}[!htb]
 ...
 \end{figure}
An FD as in Fig. (\ref{f:FDchrom}),  with $r=L/L_{IP}$ and $r_{im}=L_{IM}/L_{IP}$, has peak $\beta$-functions at the quadrupoles equal to $\beta_{x0\atop y0}=\frac{L^2_{IP}}{\beta^*_{x\atop y}}$  for $QD0$ and $
\beta_{x1\atop y1}=\beta_{x0\atop y0}\left(1+r\pm\sqrt{\frac{r}{r_{im}}+r+\frac{r^2}{r_{im}}}\sqrt{\frac{1+r}{1+r/r_{im}}}\right)^2
$ for $QF1$, using $\left(\frac{\beta^*}{L_{IP}}\right)^2\ll1$.\par
The factor $\beta_{x1\atop y1}/\beta_{x0\atop y0}$ is the beta function at QF1 relative to QD0.\par
 In the thin lens approximation, chromaticity in the horizontal or vertical plane $\xi_{x\atop y}$ can be evaluated as $\xi_{x\atop y}=\mp\frac{1}{4\pi}\left(\beta_{x1\atop y1}k_1l_1-\beta_{x0\atop y0}k_0l_0\right)=\frac{1}{4\pi}\frac{L_{IP}}{\beta^*_{x\atop y}}\Xi_{x\atop y}(r,r_{im})
$, 
where $k,l$ are the quadrupole gradient and length, $\beta^*$ is the $\beta$ function at the IP and 
%  \begin{align*}
...
\end{equation}}\par
Figure (\ref{f:figT}) show the chromaticity $\Xi_x$ and $\Xi_y$ in $\frac{1}{4\pi}L_{IP}/\beta^*$ units, and the ratio of the beta functions at the quadrupoles.\par
...
\begin{equation}
 L\propto\frac{1}{\sigma_x\sigma_y}=\frac{1}{\sigma_{x0}\sigma_{y0}\sqrt{1+(\xi^2_x+\xi^2_y)\sigma_\delta^2+\xi_x^4\xi_y^4\sigma_\delta^4}}
\end{equation}
One possibility is to minimize the addition of the horizontal and vertical chromaticities,
\begin{align}
 \xi = \xi_x + \xi_y &= \frac{1}{4\pi}\frac{L_{IP}}{\beta^*_y}\left(\frac{\Xi_x(r,r_{im})}{\beta^*_x/\beta^*_y}+\Xi_y(r,r_{im})\right)\\
 &= \frac{1}{4\pi}\frac{L_{IP}}{\beta^*_y}\Xi(\beta^*_x/\beta^*_y,r,r_{im})
\end{align}
Figure (\ref{f:fig_3TeV}) shows $\Xi$ for CLIC 3 TeV, $\beta^*_x/\beta^*_y=6.9\text{mm}/0.068\text{mm}=101$, and Fig. (\ref{f:fig_500GeV}) it the equivalent for CLIC 500 GeV \cite{CLICdes}, $\beta^*_x/\beta^*_y=8.0\text{mm}/0.1\text{mm}=80$.\par Given $L_{IM}$, the minimum added chromaticity in the FD is found when $L$ is one to two times the distance to the IP. In addition, Figures (\ref{f:fig_3TeV}-\ref{f:fig_500GeV}) show the effect of scaling up the system. For example, starting at $L=L_{IP}$ and $L_{IM}=10L_{IP}$, it is clear that a system with $L=10L_{IP}$ and $L_{IM}=100L_{IP}$, will decrease the vertical chromaticity by increasing the horizontal chromaticity.
\begin{figure}
...
Due to the low-$\beta^*$ values required for linear colliders it is necessary to cancel out the chromaticity effect. Two methods have been used: non-local and local correction.??? Needs a rewriting , large chromaticity is generated
\begin{table}[!htb]
\scriptsize
\centering
\begin{tabular}{|l|c||c|c|c|c|}\hline
...
\begin{figure}[!htb]
   \centering
   \includegraphics*[scale=0.13,angle=0]{localcorr.pdf}
   \caption{Local chromaticity correction.}
   \label{f-local}
\end{figure}
Both schemes have been compared for CLIC \cite{PhysRevSTAB.17.101001}, concluding in %smaller lattice length for the local
%scheme and 
...
\begin{figure}[!htb]
   \centering
   \includegraphics*[scale=0.13,angle=0]{noninterleavedcorr.pdf}
...
Two solutions are foreseen : the cancellation of the second order dispersion and its derivative before the FD, or alternatively generate dispersion in the FD to cancel the residual second and third order components.\par